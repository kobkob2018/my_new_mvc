		<div class='buy_leads_wrap'>
			<div class='buy_leads_leadQry'>
				...
				<h4>יתרתך: <span id="leadQry"><?php echo $this->user['lead_credit']; ?></span> לידים</h4>
			...
			<?php else: ?>
				...
				<form action='credits/sendToYaad/' method='post' name='sendto_buy_leads' id='sendto_buy_leads_form'>
					...
					<div class='buy_leads_desc form-group'>
						...
						<?php endif; ?>
						מחיר כל ליד הינו: <?php echo $this->user['lead_price']; ?> ש"ח כולל מע"מ.<br>
						<br>
				...
				<script type="text/javascript">
					...
					if(!isNaN(parseInt(l_first_num)) && isFinite(l_first_num)){
						$("#buy_leads_amount_holder").html(parseInt(l_first_num)*<?php echo $this->user['lead_price']; ?>);
						$("#buy_leads_amount_wrap").show();
					}				
					$("#num_credit_input").keyup(function(){
						var l_num = $(this).val();
						if(!isNaN(parseInt(l_num)) && isFinite(l_num)){
							$("#buy_leads_amount_holder").html(parseInt(l_num)*<?php echo $this->user['lead_price']; ?>);
							$("#buy_leads_amount_wrap").show();
						}
						else{
