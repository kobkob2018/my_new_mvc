  ...
  class LeadsController extends Controller{
    ...
    public function leadList() {
		if(isset($_REQUEST['recordings_login']) && isset($_REQUEST['recording_pass'])){
			...
			if($user['enableRecordingsPass'] == $_REQUEST['recording_pass']){
				$_SESSION[$this->session_prefix.'_recordings_pass'] = '1';
				$_SESSION[$this->session_prefix.'_show_row'] = $_REQUEST['row_id'];
				$this->redirect_to(outer_url('leads/all/'));
				return;
			}
		}
		...
		if(isset($_SESSION[$this->session_prefix.'_show_row'])){
			$show_row = $_SESSION[$this->session_prefix.'_show_row'];
			unset($_SESSION[$this->session_prefix.'_show_row']);
		}
	...
	public function get_filter(){
		...
		if(isset($_REQUEST['leads_filter'])){
			...
			$_SESSION[$this->session_prefix.'_leads_filter'] = $filter;
		}
		elseif(isset($_SESSION[$this->session_prefix.'_leads_filter'])){
			foreach($_SESSION[$this->session_prefix.'_leads_filter'] as $filter_key=>$filter_val){
				$filter[$filter_key] = $filter_val;
			}
		}	
