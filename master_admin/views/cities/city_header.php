<div class="sub-header">
    ...
    <?php if(isset($this->data['item_parent_tree'])): ?>
        <h3>ניהול קטגוריה
            ...
            <?php foreach($this->data['item_parent_tree'] as $parent_item): ?>
                <?php if($parent_item['is_current']): ?>
                    <?= $parent_item['label'] ?>
                <?php else: ?>
                    <a href = "<?= inner_url("cities/list/") ?>?item_id=<?= $parent_item['id'] ?>">
                        <?= $parent_item['label'] ?>
                    </a>
                    > 
                <?php endif; ?>
        ...
        <div class="item-edit-menu">
            <a href = "<?= inner_url('cities/list/') ?>?item_id=<?= $this->data['current_item_id'] ?>" class="item-edit-a <?= $view->a_class('cities/list/') ?>">תתי קטגוריה</a>
            | 
